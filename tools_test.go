...
// create a custom http client that has overridden functionalty
...
var jsonTests = []struct {
	...
	{name: "unknown field", json: `{"foos": "1"}`, errorExpected: true, maxSize: 1024, allowUnknown: false},
	{name: "allow unknown fields", json: `{"foos": "1"}`, errorExpected: false, maxSize: 1024, allowUnknown: true},
	{name: "missing field name", json: `{jack: "1"}`, errorExpected: true, maxSize: 1024, allowUnknown: true},
	{name: "file too large", json: `{"foos": "bar"}`, errorExpected: true, maxSize: 5, allowUnknown: true},
...
func TestTools_UploadFiles(t *testing.T) {
	for _, e := range uploadTests {
		...
		if !e.errorExpected && err != nil {
			t.Errorf("%s: error expected but none recevied", e.name)
		}
...
func TestTools_DownloadStaticFile(t *testing.T) {
	...
	testTool.DownloadStaticFile(rr, req, "./testdata", "pic.jpg", "dog.jpg")
	...
	if res.Header["Content-Disposition"][0] != "attachment; filename=\"dog.jpg\"" {
...
func TestTools_ReadJSON(t *testing.T) {
	...
	for _, e := range jsonTests {
		...
		if !e.errorExpected && err != nil {
			t.Errorf("%s: error not expect, but one received: %s", e.name, err.Error())
...
func TestTools_ErrorJSON(t *testing.T) {
	...
	if err != nil {
		t.Errorf("failed to create error json message: %v", err)
	}

	// look at the response written to the httptest recorder
